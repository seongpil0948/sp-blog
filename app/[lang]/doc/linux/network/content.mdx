# Linux Networking
## Introduction
내용이 좀 많아져서 [network-container](https://www.peachhub.love/doc/linux/network-container)
로 분리했습니다.
...
```
이제 한국 시간이 되었습니다.
아래 필드들이 yes가 아니라면, 다음 명령을 실행하여 시간 동기화를 활성화할 수 있습니다.
```bash
...
# systemd-timesyncd.service active: yes
```
#### 그래서 NTP(Network Time Protocol)가 뭔데?
지연이 있을 수 있는 네트워크 상에서, 컴퓨터와 컴퓨터간의 시간을 동기화 하기 위한 네트워크 프로토콜이다.
