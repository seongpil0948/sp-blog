...
### step for hostname to ip
1. localhost dns cache
2. using `/etc/hosts` or `C:\Windows\System32\drivers\etc\hosts` or `/etc/nsswitch.conf`
3. using `dns` server
...
``` 
위 코드는 `example.com` 을 resolve 하기 위해서 `8.8.8.8` 의 nameserver 를 사용하도록 설정한다.  
