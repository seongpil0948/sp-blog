...
## 사용자 및 그룹 디스크 할당
100테라바이트의 저장 공간을 갖춘 서버를 상상해 보세요. 그 정도면 충분합니다.   
하지만 100명의 사용자가 이 서버를 공유한다면 어떨까요? 그리고 한 사용자가 80테라바이트의 데이터를 저장한다면 어떻게 될까요?   
단 한 사람이 사용 가능한 리소스의 80%를 사용하기 때문에 이는 불공평합니다.  

이 문제를 해결하기 위해 리눅스는 사용자와 그룹에 대한 디스크 할당(disk quotas)을 제공합니다.
100명의 사용자에게 공평히 1테라바이트의 저장 공간을 할당하고 싶다면 어떻게 해야 할까요? Let's 기릿~!  
