...
관리 작업을 수행하려면 LDAP의 루트 사용자를 변경해야 합니다. LDAP 항목에서는 이 사용자를 olcRootDN이라고 합니다.  
...
# input adm
slappasswd
{SSHA}U/HJLDZqSyJnMs+i25qw7b0SUzWVKSzo
```
...
changetype: modify
add: olcRootPW
olcRootPW: {SSHA}U/HJLDZqSyJnMs+i25qw7b0SUzWVKSzo
```
...
```
5. LDAP 서버에 대한 액세스를 구성하고 서버에 대한 Manager 사용자를 구성합니다. 
```bash
...
{SSHA}aeJB3935hNMhk4ZjC12WmcMEkZEsgcV3
```
...
userPassword: {SSHA}aeJB3935hNMhk4ZjC12WmcMEkZEsgcV3
```
세진 유저를 등록합니다.
```bash
