...
ssh key를 관리 한다던가 API key를 관리하다던가 대부분 로컬에 저장되어야 한다.  
...
```bash
# a very small block of data to the storage device called a header, and by reading that header cryptsetup can automatically detect the encryption type.
