...
사용자 `sp`에게 `/mnt/backup/sp` 디렉토리에 대한 읽기 권한을 부여하고 싶다면?  
```bash
$ sudo setfacl -m u:sp:r /mnt/backup/sp
```
다시 쓰기 권한을 부여하고 싶다면?  
```bash
$ sudo setfacl -m u:sp:rw /mnt/backup/sp
```
...
```bash
$ sudo setfacl -m g:family:w /etc/hosts
```
