...
- [Practice](#practice)
    ...
    - [Tell `autofs` to auto-mount a directory called `/shares`. If this directory does not exist, `autofs` should automatically create it. The `/shares` directory should be defined in a file called `/etc/auto.shares`.](#tell-autofs-to-auto-mount-a-directory-called-shares-if-this-directory-does-not-exist-autofs-should-automatically-create-it-the-shares-directory-should-be-defined-in-a-file-called-etcautoshares)
  - [또한 autofs는 /shares/mynetworkshare/에 네트워크 공유 127.0.0.1:/etc를 자동으로 마운트하고 /shares/mynetworkshare/ 디렉토리에 127.0.0.1:/etc의 내용을 표시해야 합니다.](#또한-autofs는-sharesmynetworkshare에-네트워크-공유-127001etc를-자동으로-마운트하고-sharesmynetworkshare-디렉토리에-127001etc의-내용을-표시해야-합니다)
...
### POD 접속 테스트
```sh
$ kubectl exec -it common-mysql-59fb8bf9c7-77m9k --namespace db --container mysql -- bash
$ mysql -uroot -p
```
패스워드를 직접 입력하자. 참고로 decoded 된 평문을 입력 해야한다. `1234qwer!!`
